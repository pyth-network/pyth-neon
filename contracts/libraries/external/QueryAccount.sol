...
library QueryAccount {
    ...
    function data(bytes32 solana_address, uint64 offset, uint64 len) public view returns (bytes memory) {
        (bool success, bytes memory result) = precompiled.staticcall(abi.encodeWithSignature("data(uint256,uint64,uint64)", uint256(solana_address), offset, len));
